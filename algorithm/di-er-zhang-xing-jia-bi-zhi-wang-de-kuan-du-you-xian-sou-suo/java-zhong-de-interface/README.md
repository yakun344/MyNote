...
Map用于存储具有映射关系的数据。Map中存了两组数据(`key`与`value`),它们都可以是**任何**`引用类型`的数据，`key`不能重复，我们可以通过`key`取到对应的`value`。Map的实现类在面试中常用是：**HashMap** 和 **TreeMap**.
...
* **LinkedList** 与 **ArrayList** 对比：
  ...
  * 对于新增和删除操作`add`和`remove`，LinedList比较占优势，因为ArrayList要移动数据
