...
class Solution {
    public int myAtoi(String s) {
        ...
        while (left < s.length()) {
            ...
            if (Character.isLetter(s.charAt(left)) || s.charAt(left) == '.') {
                return 0;
            }
            if (s.charAt(left) == '-' || s.charAt(left) == '+') {
                sign = s.charAt(left) == '+' ? true : false;
                left++;
                break;
            }
            if (Character.isDigit(s.charAt(left))) break;
            left++;
        }
        while (left < s.length()) {
            if (!Character.isDigit(s.charAt(left))) break;
            if (Integer.MAX_VALUE / 10 < res ||
               Integer.MAX_VALUE / 10 == res &&
                Integer.MAX_VALUE % 10 < s.charAt(left) - '0') {
                if (sign) {
